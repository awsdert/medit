...
int _iupHList_OnButton( Ihandle *ih, int button, int pressed, int x, int y, char* status );
...
int _iupHList_OnButton( Ihandle *ih, int button, int pressed, int x, int y, char* status )
{
  ...
  {
    px += x;
    strcpy_s( attr, 20, IupGetAttribute( ih, "CLIENTOFFSET" ) );
    ...
    for ( ; i < c; ++i )
    {
      ...
      strcpy_s( attr, 20, IupGetAttribute( fset, "POSITION" ) );
      tmp = strtok_s( attr, "x", &tok );
      px = cx + atoi( tmp );
...
int _iupHList_OnIdle( Ihandle *ih )
{
  int prvValue = IupGetInt( ih, "_IUP_PRV_VALUE"), prvCount = IupGetInt( ih, "_IUP_PRV_COUNT" );
  ...
  else
  {
    ...
    for ( i = 1; i <= prvCount; ++i )
    {
      if ( !IupGetAttributeId( ih, NULL, i ) )
        break;
      IupSetAttributeId( ih, NULL, i, IupGetAttribute( IupGetChild( IupGetChild( layout, i - 1 ), 0 ), IUP_TITLE ) );
    }
    curCount = prvCount - i;
    for ( ; i <= prvCount; ++i )
    {
...
int _iupHList_OnValueChanged( Ihandle *ih )
{
  ...
  for ( i = 0; i < c; ++i )
    _iupHList_OnAction( ih, IupGetAttributeId( ih, NULL, i ), i, ( i == v ) ? 1 : 0 );
  if ( func )
    func( ih );
  return IUP_DEFAULT;
}
