...
HMODULE injectModule( HANDLE proc, LPVOID module )
{
  ...
  distantModuleMemorySpace = VirtualAllocEx( proc, NULL, moduleSize,
      ...
      if ( datadir->Size > 0 && datadir->VirtualAddress > 0 )
      {
        ...
        while( reloc->VirtualAddress != 0 )
        {
          ...
          if ( reloc->SizeOfBlock >= sizeof( IMAGE_BASE_RELOCATION ) )
          {
            ...
            LPWORD relocDescList = ( LPWORD )( ( LPBYTE )reloc + sizeof(
            ...
            for ( i = 0; i < relocDescNb; i++ )
            {
              if ( relocDescList[i] > 0 )
              {
                ...
                DWORD_PTR *p = ( DWORD_PTR * )( tmpBuffer + ( reloc->VirtualAddress +
                                                ( 0x0FFF & ( relocDescList[i] ) ) ) );
        ...
        tmpBuffer[( DWORD )main - ( DWORD )module] =
          0x55; // put the normal push ebp instruction

        /* Write processed module image in target process memory */
        ok = WriteProcessMemory( proc, distantModuleMemorySpace, tmpBuffer, moduleSize,
                                 NULL );
      }
